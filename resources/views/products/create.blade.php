      ...
      <div class="col-6">
				<div class="form-group">
					...
					<input type="text" name="desc" id="desc"
						class="form-control {{ $errors->has('desc') ? 'is-invalid' : null }}"
						value="{{ old('desc') }}">
					<span class="invalid-feedback">
						@if ($errors->has('desc'))
							{{ $errors->first('desc') }}
				...
				<div class="form-group">
					...
					>
						...
						@foreach ($status as $oneStatus)
							<option value="{{ $oneStatus->id }}"
								{{ old('status_id') == $oneStatus->id ? 'selected' : null }}
							>
								{{ $oneStatus->name }}
							</option>
						@endforeach
					...
					<span class="invalid-feedback">
						{{ $errors->has('status_id') ? $errors->first('status_id') : null}}
					</span>
				</div>
			</div>
